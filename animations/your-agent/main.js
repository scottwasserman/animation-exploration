...
window.addEventListener('pointermove', (event) => {
  if (isDragging) {
    ...
    if (shiftDrag) {
      // subtle pan via look offset — keep simple: tilt
...
window.addEventListener('pointerdown', (event) => {
  if (event.button === 0) {
    ...
    lastY = event.clientY;
    pulse = 1;
    attention = Math.min(1, attention + 0.45);

    // Thought in → response out
...
function animate() {
  ...
  attention = THREE.MathUtils.lerp(attention, isDragging ? 0.85 : 0.25 + pulse * 0.5, 0.04);
  ...
  // Core animation
  const breathe = 1 + Math.sin(elapsed * 1.4) * 0.025 + pulse * 0.08;
  core.scale.setScalar(breathe);
  coreShell.scale.setScalar(breathe * 1.02);
  coreShell.rotation.y = elapsed * 0.15;
  coreShell.rotation.x = elapsed * 0.08;
  nucleus.scale.setScalar(0.9 + Math.sin(elapsed * 2.2) * 0.12 + pulse * 0.25);
  nucleus.rotation.y = -elapsed * 0.4;

  glow.scale.setScalar(1 + pulse * 0.35 + attention * 0.15);
  glow.material.opacity = 0.14 + pulse * 0.2 + attention * 0.08;
  ...
  // Ambient streams
  if (elapsed - lastAmbientSpawn > 0.35) {
