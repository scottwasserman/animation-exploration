import"./modulepreload-polyfill-B5Qt9EMX.js";import{S as J,b as y,F as tt,c as et,W as at,V as G,a as ot,D as rt,P as nt,M as it,d as st,e as lt,L as ct,f as ft,g as ut,B as ht,C as mt}from"./three.module-B1NyhNvA.js";const pt=`
uniform float uTime;
uniform vec2 uMouse;
...
`;function N(e,t,a){const o=Math.max(0,Math.min(1,(a-e)/(t-e)));return o*o*(3-2*o)}function D(e,t,a,o,r){const n=Math.sin(t*o-e*a+r),l=Math.sin(t*o*1.73+e*a*.67+r*1.4)*.55,i=Math.sin(t*o*.41-e*a*1.35+r*2.1)*.3;return n+l+i}function F(e,t){return N(t,-2,e)}function q(e,t,a,o,r){const i=e%28;if(i>11)return{wall:0,impact:0};const c=-44+54*(i/11)**.85,h=c-a,u=Math.sin(t*.22+e*.35)*.35,m=Math.exp(-(((h+u)/7)**2)),g=Math.exp(-(((h+u)/2.4)**2))*1.4,w=(m*2.2+g)*N(14,-2,h),p=Math.hypot(t-o,a-r),x=Math.exp(-(((c-r)/4)**2))*Math.exp(-p*.12)*w*3.5;return{wall:w,impact:x}}function dt(e,t,a,o,r){const n=D(e,t-o,2.1,.62,a*.11),l=D(e,o-t,2.35,.58,a*.13+1.7),i=D(e,a-r,1.85,.55,t*.09),s=D(e,r-a,2.05,.6,t*.12+2.3),c=t-o,h=a-r,u=F(-c,13),m=F(c,13),g=F(h,13),w=F(-h,13),p=.7,z=Math.max(0,n)*Math.max(0,l),x=Math.max(0,i)*Math.max(0,s),k=z*p+x*p*.85+z*x*1.8,{wall:b,impact:v}=q(e,t,a,o,r);return n*u*p*.5+l*m*p*.5+i*g*p*.42+s*w*p*.42+k+b+v}class gt{constructor(){this.prevHeight=0,this.prevVelocity=0,this.prevImpact=0,this.cooldown=0}update(t,a,o,r,n){const l=dt(t,a,o,a,o),i=(l-this.prevHeight)/Math.max(r,.001),{impact:s}=q(t,a,o,a,o);this.cooldown-=r;const c=this.prevVelocity>.04&&i<=0&&l>.45&&this.cooldown<=0,h=s>.6&&s>=this.prevImpact&&this.prevImpact<s*.92&&this.cooldown<=0;if(c||h){const u=Math.min(1,h?s/2.8:l/2.2);n(u,h),this.cooldown=h?.35:1.1}this.prevHeight=l,this.prevVelocity=i,this.prevImpact=s}}class xt{constructor(){this.ctx=null,this.master=null,this.bedGain=null,this.bedNode=null,this.enabled=!1,this.muted=!0,this.volume=.75,this.maxGain=.42}async init(){return this.enabled?!0:(this.ctx||(this.ctx=new AudioContext,this.master=this.ctx.createGain(),this.master.gain.value=0,this.master.connect(this.ctx.destination)),this.ctx.state!=="running"&&await this.ctx.resume(),this.ctx.state!=="running"?!1:(this.bedNode||this.startBed(),this.enabled=!0,!0))}startBed(){if(!this.ctx||this.ctx.state!=="running")return;const t=this.ctx.sampleRate*3,a=this.ctx.createBuffer(1,t,this.ctx.sampleRate),o=a.getChannelData(0);let r=0;for(let l=0;l<t;l+=1){const i=Math.random()*2-1;r=r*.98+i*.15,o[l]=r*.35}this.bedNode=this.ctx.createBufferSource(),this.bedNode.buffer=a,this.bedNode.loop=!0;const n=this.ctx.createBiquadFilter();n.type="lowpass",n.frequency.value=280,n.Q.value=.6,this.bedGain=this.ctx.createGain(),this.bedGain.gain.value=.08,this.bedNode.connect(n),n.connect(this.bedGain),this.bedGain.connect(this.master),this.bedNode.start()}playPeak(t=.5,a=!1){if(!this.enabled||this.muted||!this.ctx||this.ctx.state!=="running")return;const o=this.ctx.currentTime,r=Math.max(.15,Math.min(1,t));if(a){this.playSurgeSound(o,r);return}const n=1.5+r*.8;this.playNoiseWash(o,n,r),this.playToneSwell(o,n,r)}playSurgeSound(t,a){const o=3.2+a*.8,r=Math.floor(this.ctx.sampleRate*o),n=this.ctx.createBuffer(1,r,this.ctx.sampleRate),l=n.getChannelData(0);for(let v=0;v<r;v+=1){const E=v/r,K=Math.sin(Math.PI*E)**.65;l[v]=(Math.random()*2-1)*K}const i=this.ctx.createBufferSource();i.buffer=n;const s=this.ctx.createBiquadFilter();s.type="bandpass",s.frequency.value=180+a*90,s.Q.value=.35;const c=this.ctx.createGain(),h=.26*a;c.gain.setValueAtTime(1e-4,t),c.gain.exponentialRampToValueAtTime(h,t+.35),c.gain.exponentialRampToValueAtTime(1e-4,t+o),i.connect(s),s.connect(c),c.connect(this.master),i.start(t),i.stop(t+o+.05);const u=this.ctx.createOscillator();u.type="triangle",u.frequency.setValueAtTime(38+a*12,t),u.frequency.exponentialRampToValueAtTime(24,t+o*.7);const m=this.ctx.createGain();m.gain.setValueAtTime(1e-4,t),m.gain.exponentialRampToValueAtTime(.14*a,t+.08),m.gain.exponentialRampToValueAtTime(1e-4,t+o*.85);const g=this.ctx.createBiquadFilter();g.type="lowpass",g.frequency.value=120,u.connect(g),g.connect(m),m.connect(this.master),u.start(t),u.stop(t+o+.05);const w=Math.floor(this.ctx.sampleRate*.18),p=this.ctx.createBuffer(1,w,this.ctx.sampleRate),z=p.getChannelData(0);for(let v=0;v<w;v+=1){const E=v/w;z[v]=(Math.random()*2-1)*(1-E)**2}const x=this.ctx.createBufferSource();x.buffer=p;const k=this.ctx.createBiquadFilter();k.type="highpass",k.frequency.value=900;const b=this.ctx.createGain();b.gain.setValueAtTime(.08*a,t),b.gain.exponentialRampToValueAtTime(1e-4,t+.18),x.connect(k),k.connect(b),b.connect(this.master),x.start(t),x.stop(t+.25)}playNoiseWash(t,a,o){const r=Math.floor(this.ctx.sampleRate*a),n=this.ctx.createBuffer(1,r,this.ctx.sampleRate),l=n.getChannelData(0);for(let u=0;u<r;u+=1){const m=u/r,g=Math.sin(Math.PI*m)**1.1;l[u]=(Math.random()*2-1)*g}const i=this.ctx.createBufferSource();i.buffer=n;const s=this.ctx.createBiquadFilter();s.type="bandpass",s.frequency.value=620+o*320,s.Q.value=.45;const c=this.ctx.createGain(),h=.14*o;c.gain.setValueAtTime(1e-4,t),c.gain.exponentialRampToValueAtTime(h,t+.12),c.gain.exponentialRampToValueAtTime(1e-4,t+a),i.connect(s),s.connect(c),c.connect(this.master),i.start(t),i.stop(t+a+.05)}playToneSwell(t,a,o){const r=this.ctx.createOscillator();r.type="sine",r.frequency.setValueAtTime(92+o*55,t),r.frequency.exponentialRampToValueAtTime(72,t+a);const n=this.ctx.createGain(),l=.06*o;n.gain.setValueAtTime(1e-4,t),n.gain.exponentialRampToValueAtTime(l,t+a*.35),n.gain.exponentialRampToValueAtTime(1e-4,t+a);const i=this.ctx.createBiquadFilter();i.type="lowpass",i.frequency.value=520,r.connect(i),i.connect(n),n.connect(this.master),r.start(t),r.stop(t+a+.05)}setMuted(t){this.muted=t,this.applyGain()}setVolume(t){this.volume=Math.max(0,Math.min(1,t)),this.volume===0&&(this.muted=!0),this.applyGain()}applyGain(){if(!this.master||!this.ctx)return;const t=this.muted?0:this.volume*this.maxGain;this.master.gain.cancelScheduledValues(this.ctx.currentTime),this.master.gain.setTargetAtTime(t,this.ctx.currentTime,.12)}}const H=document.getElementById("overlay"),U=document.getElementById("sound-control"),L=document.getElementById("sound-toggle"),d=document.getElementById("sound-volume"),wt=d.querySelector(".sound-volume-fill"),Mt=d.querySelector(".sound-volume-track"),Ct=d.querySelector(".sound-volume-thumb"),R=new J,V=new y(1714248);R.fog=new tt(V,18,65);R.background=zt(V);const C=new et(48,window.innerWidth/window.innerHeight,.1,120);C.position.set(0,2.2,14);C.lookAt(0,.4,-28);const S=new at({antialias:!0,alpha:!1,powerPreference:"high-performance"});S.setPixelRatio(Math.min(window.devicePixelRatio,2));S.setSize(window.innerWidth,window.innerHeight);document.body.appendChild(S.domElement);const M={uTime:{value:0},uMouse:{value:new G(.5,.5)},uPulse:{value:0},uCrashX:{value:0},uCrashZ:{value:-12},uLeftColor:{value:new y(16727327)},uRightColor:{value:new y(1743871)},uCrashColor:{value:new y(13660415)},uHorizonSurgeColor:{value:new y(11071231)},uFogColor:{value:V},uFogNear:{value:18},uFogFar:{value:65}},kt=new ot({vertexShader:pt,fragmentShader:vt,uniforms:M,side:rt});S.capabilities.isWebGL2===!1&&(H.querySelector("p").textContent="WebGL is required to view this animation.");const B=new nt(90,90,220,220);B.rotateX(-Math.PI/2);B.translate(0,0,-12);const bt=new it(B,kt);R.add(bt);Tt(R,V);const _=new G(.5,.5),Z=new G(.5,.5);let A=0,X=!1;const yt=new gt,f=new xt;function St(e){const t=Math.max(0,Math.min(100,e));wt.style.height=`${t}%`,Ct.style.bottom=`${t}%`,d.setAttribute("aria-valuenow",String(Math.round(t)))}function O(e){const t=Mt.getBoundingClientRect(),a=(1-(e.clientY-t.top)/t.height)*100;return Math.round(Math.max(0,Math.min(100,a)))}function P(){const e=f.muted||!f.enabled,t=Math.round(f.volume*100);U.classList.toggle("is-muted",e),L.setAttribute("aria-pressed",String(!e)),L.setAttribute("aria-label",e?"Turn sound on":"Turn sound off"),St(t)}async function T(e){const t=Math.max(0,Math.min(100,e))/100;t>0&&!f.enabled&&!await f.init()||(f.setVolume(t),t>0?f.setMuted(!1):f.setMuted(!0),P())}async function Q(e){e&&!f.enabled&&!await f.init()||(e&&f.volume===0&&f.setVolume(.75),f.setMuted(!e),P())}function Y(){f.enabled||Q(!0)}L.addEventListener("click",()=>{Q(f.muted||!f.enabled)});let W=!1;d.addEventListener("pointerdown",e=>{W=!0,d.setPointerCapture(e.pointerId),T(O(e))});d.addEventListener("pointermove",e=>{W&&T(O(e))});d.addEventListener("pointerup",e=>{W=!1,d.releasePointerCapture(e.pointerId)});d.addEventListener("pointercancel",()=>{W=!1});d.addEventListener("keydown",e=>{const t=e.shiftKey?10:5,a=Math.round(f.volume*100);e.key==="ArrowUp"||e.key==="ArrowRight"?(e.preventDefault(),T(a+t)):(e.key==="ArrowDown"||e.key==="ArrowLeft")&&(e.preventDefault(),T(a-t))});U.addEventListener("wheel",e=>{e.preventDefault();const t=e.deltaY>0?-5:5;T(Math.round(f.volume*100)+t)},{passive:!1});P();function j(e){_.set(e.clientX/window.innerWidth,1-e.clientY/window.innerHeight),X||(X=!0,H.classList.add("hidden"))}window.addEventListener("pointermove",j);window.addEventListener("pointerdown",e=>{A=1,Y(),j(e)});window.addEventListener("keydown",Y);window.addEventListener("resize",()=>{const{innerWidth:e,innerHeight:t}=window;C.aspect=e/t,C.updateProjectionMatrix(),S.setSize(e,t)});const I=new mt;function $(){const e=I.getDelta(),t=I.getElapsedTime();M.uTime.value=t,Z.lerp(_,.07),M.uMouse.value.copy(Z),A*=.9,M.uPulse.value=A,M.uCrashX.value=Math.sin(t*.18)*5+Math.sin(t*.05+1.2)*3+Math.sin(t*.31+2.7)*1.5,M.uCrashZ.value=-12+Math.sin(t*.15+.8)*4.5+Math.sin(t*.04)*2.5+Math.cos(t*.27+1.1)*1.8,yt.update(t,M.uCrashX.value,M.uCrashZ.value,e,(a,o)=>f.playPeak(a,o)),C.position.x=Math.sin(t*.12)*.35,C.position.y=2.2+Math.sin(t*.18)*.08,C.lookAt(0,.35,-28),S.render(R,C),requestAnimationFrame($)}$();function zt(e){const t=document.createElement("canvas");t.width=2,t.height=512;const a=t.getContext("2d"),o=a.createLinearGradient(0,0,0,t.height);o.addColorStop(0,"#060b18"),o.addColorStop(.45,"#0f1a35"),o.addColorStop(.72,e.getStyle()),o.addColorStop(1,"#243a62"),a.fillStyle=o,a.fillRect(0,0,t.width,t.height);const r=new st(t);return r.colorSpace=lt,r}function Tt(e,t){const a=new ct({color:new y(16777215).lerp(t,.35),transparent:!0,opacity:.55}),o=[];for(let n=-50;n<=50;n+=1)o.push(new ft(n,.02,-42));const r=new ut(new ht().setFromPoints(o),a);e.add(r)}
